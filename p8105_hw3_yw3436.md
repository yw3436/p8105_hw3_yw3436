...
\#\#\#\#not sure about this say the 1440 things here From the table we
built for the totals, we found that generally speaking, participants
tend to do more activity on Fridays and have the lowest activity counts
on Saturdays.
...
noaa_df = ny_noaa %>% 
  ...
  mutate(tmax = tmax/10,
         tmin = tmin/10,
         prcp = prcp/10)
## What does it mean by making sure xx are given in reasonable unit?
```
...
``` r
########should I delete these nan values?
```
